...
describe(Ingredient) do
  describe('validations for input fields, #save') do
    ...
    it('returns false when ingredient name length is <= 2 ') do
      ingredient = Ingredient.new(name: 'r')
      expect(ingredient.save).to(eq(false))
    end

    it('returns false when ingredient name length is >= 50') do
      ingredient = Ingredient.new(name: 'r'*(51))
      expect(ingredient.save).to(eq(false))
    end

    it('returns false when ingredient name is not unique') do
