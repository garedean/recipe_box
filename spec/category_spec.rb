...
describe(Category) do
  ...
  it('shows error page when category name length is <= 2 ') do
    category = Category.new(name: 'r')
    expect(category.save).to(eq(false))
  end

  it('shows error page when category name length is >= 50') do
    category = Category.new(name: 'r'*(51))
    expect(category.save).to(eq(false))
  end

  it('shows error page when category name is not unique') do
